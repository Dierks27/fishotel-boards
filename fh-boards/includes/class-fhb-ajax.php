...
class FHB_Ajax {
    ...
    public static function new_topic() {
        ...
        $topic_cat_id = isset( $_POST['topic_cat_id'] ) ? absint( $_POST['topic_cat_id'] ) : 0;

        self::require_fields( array( $title, $content, $topic_cat_id ), 'Title, message, and topic are required.' );
        ...
        if ( is_wp_error( $topic_id ) ) {
            wp_send_json_error( array( 'message' => 'Could not create thread.' ) );
        }

        update_post_meta( $topic_id, FHB_Constants::META_TOPIC_CAT_ID, $topic_cat_id );
        ...
        // Update topic_cat thread count and last activity.
        $thread_count = absint( get_post_meta( $topic_cat_id, FHB_Constants::META_THREAD_COUNT, true ) );
        update_post_meta( $topic_cat_id, FHB_Constants::META_THREAD_COUNT, $thread_count + 1 );
        update_post_meta( $topic_cat_id, FHB_Constants::META_LAST_ACTIVITY, $now );

        // Bubble activity up to the subject.
    ...
    public static function new_reply() {
        ...
        // Bubble last activity up through topic_cat → subject.
        $topic_cat_id = get_post_meta( $topic_id, FHB_Constants::META_TOPIC_CAT_ID, true );
        if ( $topic_cat_id ) {
            update_post_meta( $topic_cat_id, FHB_Constants::META_LAST_ACTIVITY, $now );
            $subject_id = get_post_meta( $topic_cat_id, FHB_Constants::META_SUBJECT_ID, true );
            if ( $subject_id ) {
    ...
    public static function search() {
        ...
        if ( ! empty( $paged_ids ) ) {
            ...
            while ( $ordered->have_posts() ) {
                ...
                $tc_id      = get_post_meta( $tid, FHB_Constants::META_TOPIC_CAT_ID, true );
                $topic_name = $tc_id ? get_the_title( $tc_id ) : '';
